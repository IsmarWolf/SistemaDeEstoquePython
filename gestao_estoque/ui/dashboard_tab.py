...
class DashboardTab(ctk.CTkFrame):
    def __init__(self, parent, db_manager, main_app):
        ...
        self.info_panel = ctk.CTkTextbox(self.filter_panel, state="disabled", wrap="word", height=180, font=("Calibri", 14))
    ...
    def export_to_pdf(self):
        ...
        try:
            fig_to_save = self.fig_pie if self.chart_type_var.get() == "Pizza" else self.fig_bar
            ...
            # Add tabulated data corresponding to the chart
            ...
            if chart_type == "Pizza" and self.product_filter_combo.get() == "Todos os Produtos":
                ...
                # Bar chart: either all products comparison or single product breakdown
                ...
                if selected_product_str == "Todos os Produtos":
                    ...
                    if product_id:
                        ...
                        for row in data:
                            # data layout used elsewhere: (day, ve, vs, qe, qs)
                            day, ve, vs, qe, qs = row[0], row[1], row[2], row[3], row[4]
    ...
    def on_hover(self, event):
        ...
        if self.chart_type_var.get() == "Pizza":
            found_wedge_index = -1
    ...
    def update_info_panel(self, item_info):
        ...
        total_gasto = financials[0] or 0; total_vendido = financials[1] or 0
        total_itens_vendidos = financials[2] or 0; estoque_atual = product_data[4]
    ...
    def _plot_all_products_comparison(self):
        ...
        if not data:
            self.ax_bar.text(0.5, 0.5, 'Sem movimentações para exibir.', ha='center', va='center', transform=self.ax_bar.transAxes); return
        ...
        for i, pid in enumerate(product_ids):
            ...
            for bar_e, bar_s in zip(bars_e, bars_s):
                self.bar_metadata.append({'bar_e': bar_e, 'bar_s': bar_s, 'pid': pid, 'name': product_names[pid]})
        if view_mode == "Valor":
    ...
    def _plot_single_product(self, product_id):
        ...
        if not data:
            self.ax_bar.text(0.5, 0.5, 'Sem movimentações para este produto.', ha='center', va='center', transform=self.ax_bar.transAxes); return
