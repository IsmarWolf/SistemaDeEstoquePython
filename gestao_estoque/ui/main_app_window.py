...
# (A classe GenericForm não muda, então não precisa ser alterada)
class GenericForm(ctk.CTkToplevel):
    ...
    def fill_form(self):
        field_map = {'Nome': 1, 'SKU': 2, 'Descrição': 3, 'Quantidade': 4, 'Preço': 5, 'CPF/CNPJ': 2, 'Telefone': 3, 'Email': 4, 'Endereço': 5, 'Contato': 2, 'Usuário': 1, 'Nível de Acesso': 2}
...
class MainAppWindow(ctk.CTk):
    def __init__(self, db_manager, user_id):
        ...
        self.create_tab("Produtos", ["ID", "Nome", "SKU", "Descrição", "Qtd", "Preço"], [50, 200, 100, 200, 80, 100], self.db_manager.get_all_products, self.open_product_form, self.open_product_form, self.delete_product)
        ...
        # <--- MELHORIA: Atualiza o gráfico após a janela ser carregada ---
        self.after(100, self.dashboard_tab_instance.update_graph)

    def create_dashboard_tab(self):
        dashboard_frame = self.tab_view.add("Dashboard")
        self.dashboard_tab_instance = DashboardTab(parent=dashboard_frame, db_manager=self.db_manager)
        self.dashboard_tab_instance.pack(fill="both", expand=True)
    ...
    # ... (O resto do arquivo não precisa de mais nenhuma alteração)
    # ... (Copie e cole a partir daqui até o final do seu arquivo original)
    ...
    def open_product_form(self, edit=False):
        ...
        fields = collections.OrderedDict([("Nome", "text"), ("SKU", "text"), ("Descrição", "text"), ("Quantidade", "number"), ("Preço", "float")])
    ...
    def save_product(self, data):
        if not all(data.get(k) for k in ["Nome", "SKU", "Quantidade", "Preço"]):
            messagebox.showerror("Erro", "Campos obrigatórios não preenchidos."); return
        try:
            if 'id' in data: self.db_manager.update_product(data['id'], data['Nome'], data['SKU'], data['Descrição'], int(data['Quantidade']), float(data['Preço']))
            else: self.db_manager.add_product(data['Nome'], data['SKU'], data['Descrição'], int(data['Quantidade']), float(data['Preço']))
            self.refresh_tab("Produtos"); self.update_movement_product_list()
        except ValueError: messagebox.showerror("Erro de Formato", "Quantidade e Preço devem ser números válidos.")
        except Exception as e: messagebox.showerror("Erro no Banco de Dados", str(e))
    ...
    def open_client_form(self, edit=False):
        ...
        if edit and not data: return
        fields = collections.OrderedDict([("Nome", "text"), ("CPF/CNPJ", "text"), ("Telefone", "text"), ("Email", "text"), ("Endereço", "text")])
    ...
    def save_client(self, data):
        if 'id' in data: self.db_manager.update_client(data['id'], data['Nome'], data['CPF/CNPJ'], data['Telefone'], data['Email'], data['Endereço'])
        else: self.db_manager.add_client(data['Nome'], data['CPF/CNPJ'], data['Telefone'], data['Email'], data['Endereço'])
    ...
    def delete_client(self):
        ...
        if data and messagebox.askyesno("Confirmar", f"Deseja excluir o cliente '{data[1]}'?"):
            self.db_manager.delete_client(data[0]); self.refresh_tab("Clientes")
    def open_supplier_form(self, edit=False):
        data = self.get_selected_item("Fornecedores") if edit else None
        if edit and not data: return
        fields = collections.OrderedDict([("Nome", "text"), ("Contato", "text"), ("Endereço", "text")])
    ...
    def save_supplier(self, data):
        if 'id' in data: self.db_manager.update_supplier(data['id'], data['Nome'], data['Contato'], data['Endereço'])
        else: self.db_manager.add_supplier(data['Nome'], data['Contato'], data['Endereço'])
    ...
    def delete_supplier(self):
        ...
        if data and messagebox.askyesno("Confirmar", f"Deseja excluir o fornecedor '{data[1]}'?"):
            self.db_manager.delete_supplier(data[0]); self.refresh_tab("Fornecedores")
    def open_user_form(self, edit=False):
        data = self.get_selected_item("Usuários") if edit else None
        if edit and not data: return
        fields = collections.OrderedDict([("Usuário", "text"), ("Senha", "password"), ("Nível de Acesso", "text")])
    ...
    def save_user(self, data):
        try:
            if 'id' in data:
                ...
                if not data['Senha']: messagebox.showerror("Erro", "A senha é obrigatória para novos usuários."); return
        ...
        finally: self.refresh_tab("Usuários")
    def delete_user(self):
        data = self.get_selected_item("Usuários")
        if data and data[0] == 1: messagebox.showerror("Erro", "Não é possível excluir o usuário administrador."); return
        if data and messagebox.askyesno("Confirmar", f"Deseja excluir o usuário '{data[1]}'?"):
            self.db_manager.delete_user(data[0]); self.refresh_tab("Usuários")
    ...
    def add_movement(self):
        ...
        if not all([prod_selection, tipo, qtd_str]): messagebox.showerror("Erro", "Todos os campos são obrigatórios."); return
        prod_id = self.product_map.get(prod_selection)
        if prod_id is None: messagebox.showerror("Erro", "Produto selecionado é inválido."); return
        try:
            qtd = int(qtd_str)
            if qtd <= 0: raise ValueError
        except ValueError: messagebox.showerror("Erro", "Quantidade deve ser um número inteiro positivo."); return
        ...
        if result == "Sucesso":
            ...
            self.refresh_tab("Movimentações"); self.refresh_tab("Produtos"); self.mov_qtd_entry.delete(0, 'end')
            self.dashboard_tab_instance.update_graph()
        else: messagebox.showerror("Erro na Movimentação", result)
